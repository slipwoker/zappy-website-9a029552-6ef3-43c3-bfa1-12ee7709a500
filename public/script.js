document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      const maxLength = parseInt(field.getAttribute('maxlength'), 10) || Infinity;
      ...
      if (type === 'email' && !isValidEmail(value)) {
        showError(field, field.getAttribute('data-error-email') || 'Please enter a valid email address.');
        return false;
      }

      if (type === 'tel' && !isValidPhone(value)) {
        showError(field, field.getAttribute('data-error-tel') || 'Please enter a valid phone number.');
        return false;
      }

      if (minLength && value.length < minLength) {
        showError(field, field.getAttribute('data-error-minlength') || 'Input is too short.');
        return false;
      }

      if (maxLength && value.length > maxLength) {
        showError(field, field.getAttribute('data-error-maxlength') || 'Input is too long.');
        return false;
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      // Allow custom submit handler via data attribute or event
      const customAction = contactForm.getAttribute('data-action');
      if (customAction) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "כרטיס צהוב-שחור משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '9a029552-6ef3-43c3-bfa1-12ee7709a500',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '9a029552-6ef3-43c3-bfa1-12ee7709a500';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '9a029552-6ef3-43c3-bfa1-12ee7709a500';
